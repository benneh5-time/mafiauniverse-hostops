...
You are [B][COLOR="#FF0000"]{player}[/COLOR][/B].

[CENTER][CHARIMG]{img_url}[/CHARIMG][/CENTER]

[I]{flavor}[/I]

You are a [B][COLOR="#FF0000"]Mafia Goon[/COLOR][/B] and have no special abilities.[/QUOTE]"""
...
def register(bot, *, mu_user_ids: dict[str, tuple[str, int]], mu_client) -> None:
    ...
    async def goon(ctx, *, args: str):
        if ctx.channel.id != ROLEPM_CHANNEL_ID:
            return
        if "|" not in args:
            await ctx.reply("Usage: `!goon <name> | <flavor>`")
            return
        name, flavor = args.split("|", 1)
        name, flavor = name.strip(), flavor.strip()
        looked_up = _lookup(name)
        if looked_up is None:
            ...
            return
        original_username, img_url = looked_up
        role = _GOON_TEMPLATE.format(player=original_username, img_url=img_url, flavor=flavor)
        output = f"{role}\n\n[HTML]{role}[/HTML]"
        try:
            reply = await asyncio.to_thread(mu_client.post_reply, ROLE_DRAFT_THREAD_ID, output)
        except Exception as exc:
